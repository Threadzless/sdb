...
use serde_json::{from_value, Error as ParseError, Value};

use crate::{record::SurrealRecord, record_link::RecordLink};
...
impl<Trg: SurrealParseTarget> SurrealParseTarget for Option<Trg> {
    fn parse(val: Value) -> Result<Self, ParseError> {
        match val {
            ...
            Value::Array(mut arr) if arr.len() != 0 => Ok(Some(Trg::parse(arr.remove(0))?)),
            Value::Object(_) => Ok(Some(Trg::parse(val)?)),
            _ => unimplemented!(),
        }
    }
}
...
impl SurrealParseTarget for String {
    fn parse(val: Value) -> Result<Self, ParseError> {
        match val {
            Value::String(s) => Ok(s),
            Value::Array(mut arr) if arr.len() != 0 => from_value(arr[0].take()),
